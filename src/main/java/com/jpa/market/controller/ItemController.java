...
public class ItemController {
    ...
    public ResponseEntity<?> itemNew(
            @Valid @RequestPart("itemCreateDto") ItemFormDto itemFormDto,
    ...
    /*
        @GetMapping
        @PostMapping
        @PutMapping     // 내용을 완전히 변경
        @PatchMapping   // 내용의 일부 변경
        @DeleteMapping  // 삭제
    */

    @PostMapping("/admin/items/{itemId}")    // 파일을 다루는 경우 Put/Patch 오류가 많아 Post 사용하는 경우 많음
    public ResponseEntity<?> itemUpdate(
        ...
        // 필수 아님
        // 프론트에서 넘어오는 dto에 id가 포함되어 있찌 않을수도 있을 경우
    ...
    public ResponseEntity<Page<ItemAdminListDto>> itemManage(

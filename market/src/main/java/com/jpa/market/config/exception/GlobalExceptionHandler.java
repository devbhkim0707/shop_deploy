...
// 컨트롤러에서 발생한 예외를 받아서
// HTTP 상태 코드 + JSON 으로 변환해주는 처리기
// @ControllerAdvice: 모든 컨트롤러의 예외를 가로채서 처리하는 클래스
// @ResponseBody + @ControllerAdvice
@RestControllerAdvice       // JSON 형태로 응답을 처리하고자 할 때 사용
public class GlobalExceptionHandler {

    // ExceptionHandler: 특정 예외 클래스를 처리하는 메서드임을 지정
    // MethodArgumentNotValidException: @Valid 유효성 검사 실패 시 throw 하는 예외
    ...
    // Map.of(): 변경할 수 없는 Map 객체를 간단하게 생성할 때 사용
